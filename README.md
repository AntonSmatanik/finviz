...
│   │   │   ├── useSearchByFullPathQuery.ts
│   │   │   ├── useSearchByNameQuery.ts
│   │   │   └── useTreeQuery.ts
...
│   │   │   ├── create.ts # Database schema creation
│   │   │   ├── index.ts  # Read operations
│   │   │   └── seed.ts   # Data seeding
│   │   ├── utils/        # Utility functions
│   │   │   ├── index.ts  # Tree operations
│   │   │   └── parse.ts  # XML parser
│   │   ├── data/         # Data files
│   │   │   └── structure_released.json
│   │   └── index.ts      # Express server
│   ├── package.json
│   └── tsconfig.json
│
└── calc/                  # XML parsing utility
    ├── files/
...
The project uses Biome for linting and formatting with the following exceptions:

- `noArrayIndexKey` - Allow array indices as React keys
- `noChildrenProp` - Allow passing children as props
- `useKeyWithClickEvents` - Allow onClick without keyboard events
- `noNoninteractiveElementToInteractiveRole` - Allow interactive static elements
- `noStaticElementInteractions` - Allow click handlers on static elements
- `noNonNullAssertion` - Allow TypeScript non-null assertions
...
- Input-styled design for better UX
- Clickable segments for navigation
- Last item visually distinct
- First item navigates to root (empty string)

### Tree Node Display

- Parent node information (name, size)
- Direct children list with indentation
- Visual tree structure with border line
- Click to navigate to child nodes
- Hover effects for better interaction

### Contact Page

- Developer information
- Skills showcase with badge design
...
- Email: smatanik.anton@gmail.com
- LinkedIn: [linkedin.com/in/anton-smatanik](https://www.linkedin.com/in/anton-smatanik)
- Location: Praha 6, Czech Republic
